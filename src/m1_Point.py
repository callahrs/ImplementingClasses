...
class Point(object):
    ...
    def move_to(self, x, y):
        temppoint = Point(self.x, self.y)
        self.x = x
        self.y = y
        self.moves = self.moves + 1
        tempdistance = self.get_distance_from(temppoint)
        self.distancetraveled = self.distancetraveled + tempdistance
    ...
    def get_distance_from(self, end):
        dist = math.sqrt(((end.x - self.x) ** 2) + ((end.y - self.y) ** 2))
        return dist

    def get_distance_from_start(self):
        newpoint = Point(self.orginalx, self.orginaly)
        return self.get_distance_from(newpoint)
